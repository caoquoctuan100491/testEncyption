...
function HUDManager() {
  throw new Error(
    "Lol, what are you doing? HUDManager is a static class. Noob."
  );
}
...
(function (_) {
  ...
  _.alertNeedSuperToolsEngine = function (update) {
    if (update) {
      alert(
        "The 'SRD_SuperToolsEngine' needs to be version 1.12 or greater to use the 'SRD_HUDMaker' plugin."
      );
    } else {
      alert(
        "The 'SRD_SuperToolsEngine' plugin is required for using the 'SRD_HUDMaker' plugin."
      );
    }
    if (
      confirm(
        "Do you want to open the download page to 'SRD_SuperToolsEngine'?"
      )
    ) {
  ...
  _.deleteKey =
    !String(params["Disable Delete Key"]).trim().toLowerCase() === "true";
  ...
  _.getPicture = function (filename, hue) {
    return ImageManager.loadBitmap("img/pictures/", filename, hue, false);
  };

  _.getGauge = function (filename, hue) {
    return ImageManager.loadBitmap(
      "img/SumRndmDde/hud/gauge_images/",
      filename,
      hue,
      false
    );
  ...
  _.getGaugeBack = function (filename, hue) {
    return ImageManager.loadBitmap(
      "img/SumRndmDde/hud/gauge_backs/",
      filename,
      hue,
      false
    );
  ...
  _.checkForCC = function () {
    if (
      Imported["SumRndmDde Character Creator"] ||
      Imported["SumRndmDde Character Creator EX"]
    ) {
      Sprite_HUDFace.prototype.refreshBitmap = function () {
        this.bitmap.clear();
        const color = _.convertHex(
          this["Background Color"],
          parseInt(this["Background Alpha"])
        );
  ...
  MapHUD.prototype._sortChildren = function () {
    ...
    this.addChildAt(
      this._highlight,
      this.children.indexOf(HUDManager.getSprite()) + 1
    );
  };
  ...
  BattleManager.isBusy = function () {
    return (
      _.BattleManager_isBusy.apply(this, arguments) ||
      $gameTemp.isManipulatingHud
    );
  };
  ...
  HUDManager.onChangeProperties = function (id) {
    ...
    MakerManager.document.getElementById("settings").innerHTML =
  ...
  HUDManager.onCreate = function () {
    ...
    const createType = String(
      MakerManager.document.getElementById("createSelect").value
    );
  ...
  HUDManager.onDelete = function () {
    ...
    this._data = datas;
    if (
      !this._sprites[this._currentId] ||
      !this._sprites[this._currentId]._isActive
    ) {
      if (this._sprites.length > 0) {
        this.refreshCurrentId();
  ...
  HUDManager.setupAnimator = function () {
    ...
    doc.getElementById("X").innerHTML = this.getAnimatorSection(
      "X",
      data.x,
      "X Offset"
    );
    doc.getElementById("Y").innerHTML = this.getAnimatorSection(
      "Y",
      data.y,
      "Y Offset"
    );
    doc.getElementById("Scale").innerHTML = this.getAnimatorSection(
      "Scale",
      data.s
    );
    doc.getElementById("Rotation").innerHTML = this.getAnimatorSection(
      "Rotation",
      data.r
    );
  };
  ...
  HUDManager.refreshAnimationSpriteAndData = function (sprite, data) {
    ...
    for (let i = 0; i < titles.length; i++) {
      for (let j = 0; j < ends.length; j++) {
        ...
        if (j === 3) {
          data.animateInfo[dataCode][ends[j]] =
            !!doc.getElementById(code).checked;
        } else {
  ...
  HUDManager.setupWindowHtml = function () {
    ...
    if (!Utils.RPGMAKER_VERSION || Utils.RPGMAKER_VERSION < "1.3.0") {
      ...
      scene.constructor === Scene_Battle
    ) {
      MakerManager.style.innerHTML = this.getStyle();
      MakerManager.window.title =
        "Super Tools Engine  -  HUD Maker  |  SumRndmDde";
  ...
  HUDManager.setupAnimatorHtml = function () {
    MakerManager.document.body.innerHTML =
      this.topBar(1) + this.getAnimatorHtml();
    this.setupAnimator();
    this.refreshChoices();
  };
  ...
  HUDManager.setupControlsHtml = function () {
    MakerManager.document.body.innerHTML =
      this.topBar(2) + this.getControlsHtml();
  };
  ...
  HUDManager.checkForUpdates = function () {
    ...
    this.getLog(
      "https://raw.githubusercontent.com/SumRndmDde/PluginUpdater/master/SRD_HUDMakerResources.json"
    );
  };
  ...
  HUDManager.setupCreditsHtml = function () {
    ...
    MakerManager.document.body.innerHTML =
      this.topBar(3) + this.getCreditsHtml();
  };
  ...
  HUDManager.getCreditsHtml = function () {
    return `<p>
				...
				<div id="scrollStuff" style="background-color: ${
          MakerManager.colors[10]
        }; height: 475px; overflow: scroll;">
					${this.getLogList()}
				<div>
				<br>
			</p>`;
  ...
  Game_Map.prototype.isEventRunning = function () {
    return (
      _.Game_Map_isEventRunning.apply(this, arguments) ||
      $gameTemp.isManipulatingHud
    );
  };
  ...
  Scene_Map.prototype.createHudBackground = function () {
    this._hudBackground = new Sprite(
      new Bitmap(Graphics.boxWidth, Graphics.boxHeight)
    );
    this._hudBackground.bitmap.fillRect(
      0,
      0,
      Graphics.boxWidth,
      Graphics.boxHeight,
  ...
  // fix
  Sprite_HUDObject.prototype.updateAnimation = function (
    mainVar,
    spd,
    loop,
    min,
  ...
  Sprite_HUDText.prototype.refreshProperties = function () {
    ...
    this.bitmap.textColor = _.convertHex(
      this["Text Color"],
      parseInt(this["Text Alpha"])
    );
    this.bitmap.outlineColor = _.convertHex(
      this["Outline Color"],
      parseInt(this["Outline Alpha"])
    );
  };
  ...
  Sprite_HUDTextEx.prototype.textColor = function (n) {
    if (this._textColorWin === undefined)
      this._textColorWin = new Window_Base();
    return this._textColorWin.textColor(n);
  };

  Sprite_HUDTextEx.prototype.drawTextEx = Window_Base.prototype.drawTextEx;
  Sprite_HUDTextEx.prototype.convertEscapeCharacters =
  ...
  Sprite_HUDTextEx.prototype.partyMemberName =
    Window_Base.prototype.partyMemberName;
  Sprite_HUDTextEx.prototype.processCharacter =
    Window_Base.prototype.processCharacter;
  Sprite_HUDTextEx.prototype.processNormalCharacter =
    Window_Base.prototype.processNormalCharacter;
  Sprite_HUDTextEx.prototype.processNewLine =
    Window_Base.prototype.processNewLine;
  Sprite_HUDTextEx.prototype.processNewPage =
    Window_Base.prototype.processNewPage;
  Sprite_HUDTextEx.prototype.obtainEscapeCode =
    Window_Base.prototype.obtainEscapeCode;
  Sprite_HUDTextEx.prototype.obtainEscapeParam =
    Window_Base.prototype.obtainEscapeParam;
  Sprite_HUDTextEx.prototype.processEscapeCharacter =
    Window_Base.prototype.processEscapeCharacter;
  Sprite_HUDTextEx.prototype.processDrawIcon =
    Window_Base.prototype.processDrawIcon;
  Sprite_HUDTextEx.prototype.makeFontBigger =
    Window_Base.prototype.makeFontBigger;
  Sprite_HUDTextEx.prototype.makeFontSmaller =
    Window_Base.prototype.makeFontSmaller;
  Sprite_HUDTextEx.prototype.calcTextHeight =
    Window_Base.prototype.calcTextHeight;
  Sprite_HUDTextEx.prototype.drawIcon = Window_Base.prototype.drawIcon;
  Sprite_HUDTextEx.prototype.changeTextColor =
  ...
  _.setupYEPMessageCore = function () {
    ...
    Sprite_HUDTextEx.prototype.convertExtraEscapeCharacters =
      Window_Base.prototype.convertExtraEscapeCharacters;
    Sprite_HUDTextEx.prototype.actorClassName =
      Window_Base.prototype.actorClassName;
    Sprite_HUDTextEx.prototype.actorNickname =
      Window_Base.prototype.actorNickname;
    Sprite_HUDTextEx.prototype.partyClassName =
      Window_Base.prototype.partyClassName;
    Sprite_HUDTextEx.prototype.partyNickname =
      Window_Base.prototype.partyNickname;
    Sprite_HUDTextEx.prototype.escapeIconItem =
      Window_Base.prototype.escapeIconItem;
    Sprite_HUDTextEx.prototype.obtainEscapeString =
      Window_Base.prototype.obtainEscapeString;
    Sprite_HUDTextEx.prototype.checkWordWrap =
      Window_Base.prototype.checkWordWrap;
    Sprite_HUDTextEx.prototype.saveCurrentWindowSettings =
      Window_Base.prototype.saveCurrentWindowSettings;
    Sprite_HUDTextEx.prototype.restoreCurrentWindowSettings =
      Window_Base.prototype.restoreCurrentWindowSettings;
    Sprite_HUDTextEx.prototype.clearCurrentWindowSettings =
      Window_Base.prototype.clearCurrentWindowSettings;
    Sprite_HUDTextEx.prototype.textWidthExCheck =
      Window_Base.prototype.textWidthExCheck;
  };
  ...
  Sprite_HUDShape.getHtml = function (data) {
    ...
    return `${HUDManager.createTitle(data.id, Sprite_HUDShape._label)}
			...
			<table>
				...
				${HUDManager.createSelect(
          "Shape",
          ["Rectangle", sele[0], "Rectangle"],
          ["Circle", sele[1], "Circle"]
        )}
				${HUDManager.createInput("Width", width)}
				${HUDManager.createInput("Height", height)}
				${HUDManager.createSelect(
  ...
  Sprite_HUDShape.prototype.refresh = function (refreshProperties) {
    ...
    this.bitmap.addLoadListener(
      function () {
        ...
        const color1 = _.convertHex(
          this["Color 1"],
          parseInt(this["Color 1 Alpha"])
        );
        const color2 = _.convertHex(
          this["Color 2"],
          parseInt(this["Color 2 Alpha"])
        );
        const color3 = _.convertHex(
          this["Outline Color"],
          parseInt(this["Color 3 Alpha"])
        );
        let color = color1;
        if (fill === "vertical") {
          ...
          color = this.bitmap._context.createRadialGradient(
            w2,
            h2,
            0,
            w2,
  ...
  Sprite_HUDGauge.prototype.refresh = function (refreshProperties) {
    ...
    if (
      ...
      (this._maxvalue || this._maxvalue == 0)
    ) {
      this.bitmap.clear();
      const outline = parseInt(this["Outline Size"]);
      ...
      const outlineCol = _.convertHex(
        this["Outline Color"],
        parseInt(this["Color 3 Alpha"])
      );
      const backCol = _.convertHex(
        this["Back Color"],
        parseInt(this["Color 4 Alpha"])
      );
      this.bitmap.fillRect(
        0,
        0,
  ...
  Sprite_HUDImageGauge.prototype.refresh = function (refreshProperties) {
    ...
    if (this.bitmap) {
      this.bitmap.addLoadListener(
        function () {
          if ($gameTemp.isManipulatingHud && this.highlight)
            this.highlight.setup(this);
          if (horizontal) {
            this._gauge.y = 0;
    ...
    if (
      ...
      this._gauge.bitmap.addLoadListener(
        function () {
          if (horizontal) {
            this._gauge._frame.height = this._gauge.bitmap.height;
            this._gauge._frame.y = 0;
            this._gauge._frame.width =
              this._gauge.bitmap.width * (this._value / this._maxvalue);
            if (style === "right") {
              this._gauge._frame.x =
                this._gauge.bitmap.width - this._gauge._frame.width;
              if (this.bitmap)
                this._gauge.x = this.bitmap.width / -2 + this._gauge._frame.x;
            } else if (style === "left") {
            ...
            this._gauge._frame.x = 0;
            this._gauge._frame.height =
              this._gauge.bitmap.height * (this._value / this._maxvalue);
            if (style === "down") {
              this._gauge._frame.y =
                this._gauge.bitmap.height - this._gauge._frame.height;
