...
function DataManagerEX() {
  throw new Error(
    "It doesn't matter how extreme, DataManagerEX is a static class! (╬Ò^Ó)"
  );
}
...
(function (_) {
  ...
  if (!params["Menu Editor Exempt List"]) {
    alert(
      "SRD_SuperToolsEngine has updated its parameter system!\nPlease reinstall the plugin or update the parameters!"
    );
    return;
  }
  ...
  if (_.isPlaytest) {
    ...
    MakerManager.moveWindow = function () {
      if (_.move) {
        window.moveBy(-300, 0);
        this._window.moveTo(
          window.screenX + Graphics.boxWidth + 6,
          window.screenY
        );
      }
    };
    ...
    MakerManager.closeTheWindows = function () {
      ...
      if (MakerManager.window) {
        if (
          window.confirm(
            "Are you sure you wish to close the game while Editor Window is active? Your changes will not be saved."
          )
        ) {
    ...
    MakerManager.setupGameWindow = function () {
      ...
      win.on("move", function (x, y) {
        if (ConfigManager.STE_Follow && MakerManager.window) {
          MakerManager.window.x =
            x + Graphics.width + ConfigManager.STE_FollowX;
    ...
    MakerManager.setupWindowHtml = function () {
      this.updateStyle();
      MakerManager.window.title =
        "Super Tools Engine  -  Core Editor  |  SumRndmDde";
      MakerManager.mode = "";
    ...
    MakerManager.getLauncherButtons = function () {
      if (this._buttons === undefined)
        this._buttons = this.getLauncherButtonsRaw();
      return this._buttons;
    };

    MakerManager.getLauncherButtonsRaw = function () {
      return [
        `${this.generateLauncherRow(
          "Database EX",
          "A tool that adds, edits, and upgrades various MV Database inputs.",
          "DataManagerEX.setupWindowHtml()",
          "#f41f1f"
        )}`,
        `${this.generateLauncherRow(
          "Debug Tool",
          "A tool containing various features for testing one's game.",
          "DebugManager.setupWindowHtml()",
          "#1f8af4"
        )}`,
        `${this.generateLauncherRow(
          "Menu Editor",
          "A tool that allows developers to edit the window setups for scenes.",
          "WindowManager.setupWindowHtml()",
          "#8a1ff4"
        )}`,
      ];
    };
    ...
    MakerManager.generateLauncherRow = function (
      name,
      description,
      method,
      color
    ...
    MakerManager.getOptionsHtml = function () {
      ...
      return `<p><table>
				...
				<tr>
					...
					<td style="text-align: center;">
						<input type="text" id="FollowX" onchange="MakerManager.updateFollow()" value="${String(
              ConfigManager.STE_FollowX
            )}">
					</td>
				</tr>
				<tr>
					...
					<td style="text-align: center;">
						<input type="text" id="FollowY" onchange="MakerManager.updateFollow()" value="${String(
              ConfigManager.STE_FollowY
            )}">
					</td>
				</tr>
			</table></p>`;
    ...
    MakerManager.updateFollow = function () {
      ConfigManager.STE_Follow =
        this._document.getElementById("Follow").checked;
      ConfigManager.STE_FollowX = parseInt(
        this._document.getElementById("FollowX").value
      );
      ConfigManager.STE_FollowY = parseInt(
        this._document.getElementById("FollowY").value
      );
      ConfigManager.save();
      if (ConfigManager.STE_Follow && MakerManager.window) {
        MakerManager.window.x =
          window.screenX + Graphics.width + ConfigManager.STE_FollowX;
    ...
    MakerManager.updateTheme = function () {
      switch (ConfigManager.STE_Theme) {
        ...
        case 8:
          ...
          for (let i = 0; i < 17; i++) {
            this._colors.push(
              `rgba(${Math.randomInt(255)}, ${Math.randomInt(
                255
              )}, ${Math.randomInt(255)}, 255)`
            );
          }
          break;
      }
  ...
  if (_.isPlaytest) {
    ...
    DataManagerEX.setupWindowHtml = function () {
      this.initOldDatas();
      MakerManager.window.title =
        "Super Tools Engine  -  Database EX  |  SumRndmDde";
    ...
    DataManagerEX.getObjectList = function () {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (obj) {
          ...
          result += `<option value="${i}" ${selected}>${_.pad(obj.id)}) ${
            obj.name
          } ${changed}</option>`;
        }
      }
    ...
    DataManagerEX.getPropertyList = function () {
      ...
      let result = "";
      for (
        let i = this._listIndex * 6;
        i < Math.min((this._listIndex + 1) * 6, list.length);
        i++
      ) {
        const name =
          list[i].substring(0, 1) === "#" ? list[i].substring(1) : list[i];
        ...
        result += `<tr>
					...
					<td style="width: 15%; text-align: center;"><input type="checkbox" id="check_${
            list[i]
          }" ${checked}></td>
					<td style="width: 55%; text-align: center;"><input type="text" size="45" id="${
            list[i]
          }" value='${value}'></td>
				</tr>`;
      }
    ...
    DataManagerEX.refreshObjects = function () {
      MakerManager.document.getElementById("ObjectSelect").innerHTML =
        this.getObjectList();
      this.refreshProperties();
    };

    DataManagerEX.refreshProperties = function () {
      MakerManager.document.getElementById("PropertyList").innerHTML =
        this.getProperties();
    };

    DataManagerEX.refreshPages = function () {
      MakerManager.document.getElementById("PageControl").innerHTML =
        this.getPages();
    };

    DataManagerEX.saveCurrent = function () {
      if (!$dataBasicEX[this._type][this._id])
        $dataBasicEX[this._type][this._id] = {};
      const list = this._props[this._type];
      const data = $dataBasicEX[this._type][this._id];
      for (let i = 0; i < list.length; i++) {
        const element = MakerManager.document.getElementById(list[i]);
        const chkElem = MakerManager.document.getElementById(
          "check_" + list[i]
        );
        const propName =
          list[i].substring(0, 1) === "#" ? list[i].substring(1) : list[i];
        if (element && chkElem && element.value.trim().length > 0) {
          ...
          typeof $dataBasicEX[this._type][this._id][propName] !== "undefined"
        ) {
    ...
    DataManagerEX.refreshBigNote = function () {
      MakerManager.document.getElementById("NoteInput").value =
        this.getCurrentNote();
    };
    ...
    DataManagerEX.saveCurrentNote = function () {
      ...
      if (
        typeof $dataNotes[this._type][this._id] === "string" &&
        $dataNotes[this._type][this._id] === ""
      ) {
    ...
    DataManagerEX.requestRestart = function () {
      if (this.requestRestartCondition()) {
        if (
          window.confirm(
            "Database EX changes are detected! Would you like to preform a SUPER RESTART to apply the changes?"
          )
        ) {
          _.move = false;
    ...
    DebugManager.refreshCollisions = function () {
      this.disableCollisions =
        !!MakerManager.document.getElementById("collision").checked;
    };

    DebugManager.refreshEncounters = function () {
      this.disableEncounters =
        !!MakerManager.document.getElementById("encounter").checked;
    };
    ...
    DebugManager.restartGame = function () {
      if (!SceneManager._sceneStarted)
        setTimeout(this.restartGame.bind(this), 10);
      if (!this.isOnMap()) {
    ...
    DebugManager.setupWindowHtml = function () {
      ...
      MakerManager.window.title =
        "Super Tools Engine  -  Debug Tool  |  SumRndmDde";
    ...
    DebugManager.getActorList = function () {
      ...
      for (let i = 0; i < $gameParty.members().length; i++) {
        ...
        if (!info) continue;
        result +=
          '<option value="' + info.actorId() + '">' + info.name() + "</option>";
      }
      result += `</select>`;
      return result;
    };
    ...
    DebugManager.getBasicHtml = function () {
      return `<p><table>
				...
				<tr>
					...
					<td style="text-align: center;">
						<input type="checkbox" style="width:50%;height:50%;" onchange="DebugManager.refreshCollisions()" id="collision" ${
              this.disableCollisions ? "checked" : ""
            }>
					</td>
					<td style="width: 40%;">
				...
				<tr>
					...
					<td style="text-align: center;">
						<input type="checkbox" style="width:50%;height:50%;" onchange="DebugManager.refreshEncounters()" id="encounter" ${
              this.disableEncounters ? "checked" : ""
            }>
					</td>
					<td style="width: 40%;">
    ...
    DebugManager.refreshSwitchTables = function () {
      ...
      for (let i = first; i <= Math.min(first + 19, this._maxSwitches); i++) {
        ...
        result1 += `<tr>
						...
						<td style="padding: 2px;width: 35%;">${
              str.length > 20 ? str.substring(0, 20) + "..." : str
            }</td>
						<td style="padding: 2px;width: 5%;" id="Switch ${i} Label">${
          value ? "ON" : "OFF"
        }</td>
						<td style="padding: 2px;width: 5%;"><input type="checkbox" onchange="DebugManager.changeSwitch(${i})" id="Switch ${i}" ${
          value ? "checked" : ""
        }></td>
					</tr>`;
      }
      for (
        let i = first + 20;
        i <= Math.min(first + 39, this._maxSwitches);
        i++
      ) {
        const value = $gameSwitches.value(i);
        const str = $dataSystem.switches[i];
      ...
      MakerManager.document.getElementById("Page").innerHTML = `Page: (${
        this._listIndex + 1
      }/${Math.ceil(this._maxSwitches / 40)})`;
    };
    ...
    DebugManager.refreshVariableTables = function () {
      ...
      for (
        let i = first + 20;
        i <= Math.min(first + 39, this._maxVariables);
        i++
      ) {
        const value = $gameVariables.value(i);
        const str = $dataSystem.variables[i];
      ...
      MakerManager.document.getElementById("Page").innerHTML = `Page: (${
        this._listIndex + 1
      }/${Math.ceil(this._maxVariables / 40)})`;
    };

    DebugManager.incrementPageExists = function () {
      const length =
        this.mode === "switch" ? this._maxSwitches : this._maxVariables;
    ...
    DebugManager.changeSwitch = function (id) {
      const value = MakerManager.document.getElementById(
        "Switch " + id
      ).checked;
      $gameSwitches.setValue(id, value);
      MakerManager.document.getElementById(
        "Switch " + id + " Label"
      ).innerHTML = value ? "ON" : "OFF";
    };

    DebugManager.changeVariable = function (id) {
      const value = Number(
        MakerManager.document.getElementById("Variable " + id).value
      );
    ...
    WindowManager.setupWindowHtml = function () {
      ...
      if (cnstr === Scene_Map) {
        ...
        cnstr === Scene_Battle &&
        Imported["SumRndmDde Battle GUI Core"]
      ) {
        alert("You cannot edit battles with SRD_BattleGUICore installed!");
      } else if (
        scene._windowLayer &&
        scene._windowLayer.children &&
        scene._windowLayer.children.length > 0
      ) {
        MakerManager.window.title =
          "Super Tools Engine  -  Menu Editor  |  SumRndmDde";
    ...
    WindowManager.topBar = function (type) {
      ...
      return `<ul style="cursor:pointer">
				<li><a ${
          active["main"] || ""
        } onclick="WindowManager.setupMakerHtml()">Maker</a></li>
    ...
    WindowManager.getTableHtml = function () {
      ...
      let result = `<tr>
					...
					<td style="width: 50%;text-align: center;"><input type="text" id="Opacity" value="${
            data.opacity || this._focusWindow.opacity
          }"></td>
				</tr>
				<tr>
					<td style="text-align: center;">Width:</td>
					<td style="width: 50%;text-align: center;"><input type="text" id="Width" value="${
            data.width || this._focusWindow.width
          }"></td>
				</tr>`;
      if (!this._focusWindow._ste_isCommand) {
        result += `<tr>
							<td style="text-align: center;">Height:</td>
							<td style="width: 50%;text-align: center;"><input type="text" id="Height" value="${
                data.height || this._focusWindow.height
              }"></td>
						</tr>`;
      }
      if (this._focusWindow._ste_isSelectable) {
        if (
          this._focusWindow._ste_isCommand ||
          this._focusWindow.numVisibleRows
        ) {
          result += `<tr>
						<td style="text-align: center;">Visible Rows:</td>
						<td style="width: 50%;text-align: center;"><input type="text" id="Rows" value="${
              data.rows || this._focusWindow.numVisibleRows()
            }"></td>
					</tr>`;
        }
        result += `<tr>
					<td style="text-align: center;">Max Columns:</td>
					<td style="width: 50%;text-align: center;"><input type="text" id="Cols" value="${
            data.cols || this._focusWindow.maxCols()
          }"></td>
				</tr>`;
      }
      if (this._focusWindow._ste_isCommand) {
        result += `<tr>
					...
					<td style="width: 50%;text-align: center;"><input type="text" id="lineHeight" value="${
            data.lineHeight || this._focusWindow.lineHeight()
          }"></td>
				</tr>`;
    ...
    WindowManager.getWindowList = function () {
      let result =
        '<select id="WindowSelect" onchange="WindowManager.onWindowChange()">';
    ...
    WindowManager.windowMeetsConditions = function (window) {
      return (
        ...
        this.windowConstructorMeetsConditions(window)
      );
    };
    ...
    WindowManager.windowConstructorMeetsConditions = function (window) {
      return (
        ...
        !_.banList.contains(window.constructor.name)
      );
    };
    ...
    WindowManager.resetScene = function () {
      if (
        confirm(
          "Would you like to reset all changes? The game will need to restart."
        )
      ) {
  ...
  if (_.isPlaytest) {
      ...
      if (!this._isWindowEditting) {
        ...
        if (this._editWindow) {
          if (TouchInput.isPressed()) {
            ...
            $dataWindows[this.constructor.name][this._editWindowName].x =
              this._editWindow.x;
            $dataWindows[this.constructor.name][this._editWindowName].y =
              this._editWindow.y;
          }
        }
      }
    ...
    Scene_Base.prototype.setEditWindow = function (name) {
      if (!$dataWindows[this.constructor.name])
        $dataWindows[this.constructor.name] = {};
      if (this._editWindow) this._editWindow.setMenuEditTarget(false);
      const children = this._windowLayer.children;
      for (let i = 0; i < children.length; i++) {
        const win = children[i];
        if (win && win.getWindowCode() === name) {
          ...
          if (!$dataWindows[this.constructor.name][this._editWindowName])
            $dataWindows[this.constructor.name][this._editWindowName] = {};
        }
      }
    ...
    Scene_Base.prototype.refreshEditSnaps = function () {
      ...
      this._editXSnaps = this._editXSnaps.filter(function (
        value,
        index,
        array
      ) {
      ...
      this._editYSnaps = this._editYSnaps.filter(function (
        value,
        index,
        array
      ) {
    ...
    Scene_Base.prototype.refreshEditWindowProperties = function () {
      ...
      const errorText =
        "Error with " + this.constructor.name + "'s " + this._editWindowName;
      if (opacity) {
  ...
  Scene_Base.prototype.initCustomWindowPositions = function () {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (win && $dataWindows[this.constructor.name][win.getWindowCode()]) {
        ...
        const errorText =
          "Error with " + this.constructor.name + "'s " + win.getWindowCode();
  ...
  Scene_ItemBase.prototype.showSubWindow = function (window) {
    ...
    if (
      $dataWindows[this.constructor.name] &&
      $dataWindows[this.constructor.name][window.getWindowCode()]
    ) {
      window.x = tempX;
    }
  };
  ...
  _.Scene_Battle_updateWindowPositions =
  ...
  Scene_Battle.prototype.disableWindowPositionUpdates = function () {
    return !!(
      $dataWindows["Scene_Battle"] &&
      $dataWindows["Scene_Battle"]["Window_BattleStatus"]
    );
  };
  ...
  if (_.isPlaytest) {
    ...
    Scene_Boot.prototype.isGameRestarting = function () {
      return (
        this._setReopener !== false &&
        StorageManager.exists(parseInt(this._setReopener))
      );
    };
    ...
    Game_Player.prototype.isDebugThrough = function () {
      return (
        _.Game_Player_isDebugThrough.apply(this, arguments) ||
        DebugManager.disableCollisions
      );
    };
    ...
    Game_Player.prototype.canEncounter = function () {
      return (
        _.Game_Player_canEncounter.apply(this, arguments) &&
        !DebugManager.disableEncounters
      );
    };
    ...
    Game_Switches.prototype.setValue = function (switchId, value) {
      ...
      if (
        DebugManager.active &&
        DebugManager.mode === "switch" &&
        switchId > 0 &&
        switchId < $dataSystem.switches.length
      ) {
        const val = this._data[switchId];
        MakerManager.document.getElementById("Switch " + switchId).checked =
          val;
        MakerManager.document.getElementById(
          "Switch " + switchId + " Label"
        ).innerHTML = val ? "ON" : "OFF";
      }
    };
    ...
    Game_Variables.prototype.setValue = function (variableId, value) {
      ...
      if (
        DebugManager.active &&
        DebugManager.mode === "variable" &&
        variableId > 0 &&
        variableId < $dataSystem.variables.length
      ) {
        const val = this._data[variableId];
        MakerManager.document.getElementById("Variable " + variableId).value =
          val;
      }
    };
